...
-   Достижение точности классификации не менее 80% за период ОПЭ.
...
1.  **Сбор и подготовка данных:**
    -   Выгрузка исторических данных из Jira с параметрами "Название заявки" и "Описание".
    -   Очистка данных:
        -   Удаление пустых и некорректных записей.
        -   Удаление шаблонных участков (например, подписей в письмах).
    -   Формирование датасета с учётом дисбаланса классов.
2.  **Предобработка текста (Word2Vec):**
    -   Удаление цифр, шаблонов и несловесных символов.
    -   Замена именованных сущностей (NER) на их типы.
    -   Приведение текста к нижнему регистру, лемматизация и удаление стоп-слов.
    -   Раскрытие аббревиатур и форматирование пробелов.
3.  **Обучение базовой модели:**
    -   Применение Word2Vec для векторизации текста.
    -   Тестирование классических алгоритмов: Logistic Regression (LR), Naive Bayes (NB), Support Vector Classifier (SVC), Random Forest (RF), Gradient Boosting (GB).
    -   Использование AutoML (AutoGluon) для поиска наилучшей модели.
4.  **Оценка результатов:**
    -   Сбор метрик качества: Accuracy, F1-Score.
    -   Анализ ошибок для выявления проблемных классов.

**Блок-схема для MVP (разработка минимального рабочего решения):**

1.  **Сбор и обновление данных:**
    -   Постоянная выгрузка данных из Jira с учётом новых заявок.
    -   Обновление словарей и переводы старых форматов классов для увеличения выборки.
    -   Применение подходов кластеризации для расширения обучающей выборки.
2.  **Сокращённая предобработка текста (для transformer моделей):**
    -   Удаление цифр, шаблонов и несловесных символов.
    -   Приведение текста к нижнему регистру и раскрытие аббревиатур.
    -   Форматирование пробелов для улучшения качества данных.
3.  **Обучение модели:**
    -  Применение DL моделей: RNN и LSTM.
    - Использование transformer моделей (DistilruBERT), проверка качества и оценка производительности.
4.  **Оценка и валидация:**
    -   Метрики качества: Accuracy, F1-Score.
    -   Анализ метрик для крупных и мелких классов.
5.  **Интеграция и развёртывание:**
    -   Создание REST API с использованием FastAPI.
    -   Обёртывание модели в Docker для удобства развёртывания.
    -   Проверка совместимости с текущей инфраструктурой Jira.
6.  **MLOps и автоматизация:**
    -   Настройка пайплайна автоматического обучения и обновления модели (ClearML).
    -   Автоматизация проверки метрик качества и публикации моделей.
7.  **Пилотирование:**
    -   Развёртывание в рабочей среде для реального тестирования.
    -   Сбор обратной связи и доработка модели на основе полученных данных за период ОПЭ.